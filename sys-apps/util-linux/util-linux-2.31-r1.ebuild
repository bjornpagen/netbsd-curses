...
SLOT="0"
IUSE="build caps +cramfs fdformat kill curses nls pam python +readline selinux slang static-libs +suid systemd test tty-helpers udev unicode"

# Most lib deps here are related to programs rather than our libs,
# so we rarely need to specify ${MULTILIB_USEDEP}.
RDEPEND="caps? ( sys-libs/libcap-ng )
	cramfs? ( sys-libs/zlib )
	curses? ( virtual/curses:0= )
	pam? ( sys-libs/pam )
	python? ( ${PYTHON_DEPS} )
